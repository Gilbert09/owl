...
**Recently landed**:
- Session 74 (external merge queues — trunk.io): a base branch can be governed by a merge system Talyn can't merge past (posthog/posthog's `master` since July 2026 — a ruleset exempts only trunk.io's App, so our merge 405s "Cannot update this protected ref"). The queue now takes the PR to green and then **submits** it to that system — arm GitHub auto-merge, else apply the repo's submit label (`trunk-merge-queue-submit`; needs the App's `issues: write`) — parks the entry in `awaiting_external`, tracks the provider's state off the PR's **labels** (trunk's only signal: `trunk-queued`/`trunk-testing`/`trunk-failed`/…), and takes the PR back to fix + resubmit if it's ejected, bounded by a per-head submit budget. Gate detection: `services/repoMergeGate.ts` (cheap branch-rules probe = `suspected`, observed 405 = `confirmed` + sticky). Gated groups always evaluate eagerly — the provider does the ordering. The desktop Merge button submits too. See Session 74 in [`docs/SESSIONS.md`](./docs/SESSIONS.md).
