...
- **Launch decisions (Session 65 audit)**: access model (allowlist vs invites — no invite flow exists); ~~stable vs prerelease update channel~~ (Session 96: the stable release ships nightly with the full arch matrix and nothing publishes pre-releases; the in-app picker is now a no-op to remove or repurpose); error-tracking/uptime alerting (process handlers log, nothing pages); `docs/SETUP.md` rewrite (predates the cloud-only refactor). ~~Repo codename exposure~~ — renamed `Gilbert09/owl` → `Gilbert09/talyn` (Session 65; GitHub redirects keep old release feeds + git remotes working — never reuse the `owl` name).
...
- **Merge stack follow-ups (Session 85)**: stack enqueue still inherits the workspace merge method, and `squash` is the hazard — a squash-landed parent leaves its commits only on the child's branch, so the retarget's `update_branch` conflicts and a paid cloud fix run does the rebase (it gets an explicit hint, but defaulting stack enqueue to `merge` would sidestep it entirely — open product call). No production exercise yet: the drain is covered end-to-end against pglite with a mocked GitHub, but nobody has pointed it at a real stack. The reconciler has no counter for entries parked in `awaiting_stack` beyond a day.

**Resolved:**
- ~~Stacked PRs have to be landed by hand~~ — Session 85: the merge queue parks each member in `awaiting_stack` behind the PR its base belongs to, retargets it onto the real base when that lands, and drains the chain bottom-up. `POST /:id/merge-queue/stack` queues the whole thing in one call.
