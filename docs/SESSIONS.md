...
Tests: `promptTemplates.test.ts` (renderer, validation, hash, overrides through both builders, the bot policy), `routes/workspaces.test.ts` (PATCH validation matrix), `prAutoMergeWatcher.test.ts` (override reaches the created task), `InstructionsSettings.test.tsx` + `promptEditor.test.ts` in both front ends.
