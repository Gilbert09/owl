...
**Still open**: the `apps/web` fork (decided: a separate forked UI, full parity), and a spike that needs real infra — `https://app.talyn.dev/auth/callback` on the Supabase redirect allowlist plus `ALLOWED_ORIGINS` on Railway — to prove browser PKCE (especially Safari, where the current post-`await` `window.open` fallback would be popup-blocked) and background-tab WS behaviour before 18k LOC gets forked. See [`docs/ROADMAP.md`](./ROADMAP.md).
