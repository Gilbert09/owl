...
describe('submitToExternalQueue', () => {
  ...
  beforeEach(() => {
    ...
    enable = vi.spyOn(autoMerge, 'enableAutoMerge');
    labels = vi
      .spyOn(githubService, 'listRepoLabelNames')
      .mockResolvedValue(['trunk-merge-queue-submit']);
    addLabels = vi.spyOn(githubService, 'addPullRequestLabels').mockResolvedValue(undefined);
  });
  afterEach(() => vi.restoreAllMocks());

  it('submits by arming GitHub auto-merge — the door that needs no extra permission', async () => {
    enable.mockResolvedValue({ armed: true });
    ...
    // The arm is pinned to the head we decided on, so a concurrent push can't
    // submit the wrong commit.
  ...
  it('falls back to the submit label when GitHub refuses to arm a clean PR', async () => {
  ...
  it('reports clean_status instead of labelling when the caller wants to try a direct merge', async () => {
    enable.mockResolvedValue({ armed: false, reason: 'clean_status' });
  ...
  it('uses the label when the repo forbids auto-merge outright', async () => {
  ...
  it('reports no_mechanism when neither door exists — the one case a human must handle', async () => {
    capability.mockResolvedValue('unavailable');
    labels.mockResolvedValue(['stamphog']);
    const result = await submitToExternalQueue(base);
    expect(result.kind).toBe('no_mechanism');
    expect(result).toMatchObject({ message: expect.stringContaining('submit label') });
  });
