import { downscaleImage } from '../renderer/lib/imageDownscale';

/**
 * Workspace-logo uploads go through this. The original version passed a blob:
 * object URL to an <img>, which the app's own CSP refused: the renderer's
 * index.html sends `img-src 'self' data: https:`, and `'self'` does not cover
